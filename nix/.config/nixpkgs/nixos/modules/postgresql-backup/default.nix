# This is the same as services.postgresqlBackup except for using cp instead of
# mv when renaming the backup file.  This is necessary for s3 rclone remotes,
# which don't support server side mv.
...
let
  ...
  cfg = config.services.local.postgresqlBackup;
  ...
  postgresqlBackupService = db :
    {
      ...
      script = ''
        if [ -e ${cfg.location}/${db}.sql.gz ]; then
          ${pkgs.coreutils}/bin/cp ${cfg.location}/${db}.sql.gz ${cfg.location}/${db}.prev.sql.gz
        fi
...
in {
  ...
  config = mkIf config.services.local.postgresqlBackup.enable {
